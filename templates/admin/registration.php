    ...
    <div class="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                ...
                <li>
                    <a href="/admin/registrations/" class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">Registrations</a>
                </li>
            </ul>
        </div>
    </div>
                    ...
                    <?php foreach ($registrations as $registration) : ?>
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <?= Event::getById($registration['event_id'])['name'] ?>
                            </th>
                            <td class="px-6 py-4">
                                <?= User::getById($registration['user_id'])['name'] ?>
                            </td>
                            <td class="px-6 py-4">
                                <?= User::getById($registration['user_id'])['phone'] ?>
                            </td>
                            <td class="px-6 py-4">
                                <?= User::getById($registration['user_id'])['email'] ?>
                            </td>
                        </tr>
                    <?php endforeach ?>
                </tbody>
